resource "kubernetes_namespace" "radeck_test" {
  metadata {
    name = "radeck-test"
    annotations = {
...
resource "kubernetes_service_account_v1" "radeck_test" {
  metadata {
    name      = "radeck-test"
    namespace = kubernetes_namespace.radeck_test.metadata[0].name
  }
}

resource "kubernetes_config_map_v1" "collector" {
  metadata {
    name      = "radeck-test-config"
    namespace = kubernetes_namespace.radeck_test.metadata[0].name
  }
  ...
  depends_on = [kubernetes_namespace.radeck_test]
}
    ...
    template {
      ...
      spec {
        container {
          ...
          #command = ["/otelcol", "--config=/conf/radeck-test-config.yaml"]
